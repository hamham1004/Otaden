...
public class IncomingCallReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
            ...
            if (callActivity.getMyFavoriteMember() == AppConstants.CATEGORY_MOMO) {
            	...
            	// 通話ログ書き込み、もも電の時はisUseは9固定
            	new InsertLogTask().execute(AppConstants.INSERT_LOG_URL, callActivity.thisDN, 
            			9, callActivity.getMessage());
            } else {
            	new GetProfileTask(callActivity).execute(AppConstants.GET_PROFILE_URL, callActivity.call.getPeerProfile().getUserName());
            	// 通話ログ書き込み
            	new InsertLogTask().execute(AppConstants.INSERT_LOG_URL, callActivity.thisDN, 
            			callActivity.getMyFavoriteMember(), callActivity.getMessage());
            }
