...
/**
 * 発信側から受信側に変わる時に、自分の推しメンをDB上の5000番のisUse値に書き込むタスク<br>
 * 引数1個目：String 送信先URL<br>
 * 引数2個め：String 書き込み先のDN<br>
 * 引数3個目：int isUseに書き込む値、推しメンを表す番号、1：箱推し、2～6：各メンバー
 */
public class PutProfileTask extends AsyncTask<Object, Void, Void> {

	// ログ出力用タグ名
	private final String TAG = "PutProfileTask";

	@Override
	protected Void doInBackground(Object... params) {
		Log.i(TAG, "doInBackground start");
		Log.d(TAG, "url=<" + params[0] + ">, dn=<" + params[1] + ">, pass=<" + params[2] + ">, comment=<" + params[3] + ">");
		...
		paramList.add(new BasicNameValuePair("comment", (String)params[3]));
		try {
