...
public class CallActivity extends Activity {
    ...
    public TextView memberText;  // 相手の一言メッセージ、GetProfileTaskから更新するからpublic
...
//  Button holdButton;
	...
	private void initParentLayout() {
        ...
        layout.setBackgroundDrawable(getResources().getDrawable(foo.bar.otaden.R.drawable.maple));
	...
	private void initMemberText() {
        ...
        LayoutParams layoutParams2 = new LayoutParams(getPixels(247), LinearLayout.LayoutParams.WRAP_CONTENT);
        layoutParams2.gravity = Gravity.CENTER;
//		layoutParams2.setMargins(getPixels(30), 0, 0, 0);
        memberTextCaption.setLayoutParams(layoutParams2);
        memberTextCaption.setText("相手の一言メッセージ");

        // 一言メッセージ表示用のtextViewを作る
        memberText = new TextView(this);
        LayoutParams layoutParams = new LayoutParams(getPixels(247), getPixels(45));
        ...
        memberText.setBackgroundColor(Color.argb(127, 255, 213, 234));
//        memberText.setGravity(Gravity.CENTER);
        memberText.setTextSize(16.0f);
	}
...
//        initHoldButton();
	...
	private void initMuteButton() {
        ...
        getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_sound_off).clearColorFilter();
        muteButton.setBackgroundDrawable(getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_sound_off));
        muteButton.setOnClickListener(new OnClickListener() {
    		public void onClick(View arg0) {
    	    	Log.i(TAG, "muteButton onClickListener start");
	    		call.toggleMute();
    			Drawable drawable = getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_sound_off);
	    		if (call.isMuted()) {
...
/*	@SuppressWarnings("deprecation")
	private void initHoldButton() {
        ...
        getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_answer_and_hold).clearColorFilter();
        holdButton.setBackgroundDrawable(getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_answer_and_hold));
        holdButton.setOnClickListener(new OnClickListener() {
    		public void onClick(View arg0) {
    	    	Log.i(TAG, "holdButton onClickListener start");
    			Drawable drawable = getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_answer_and_hold);
	    		if (call.isOnHold()) {
	    			try {
						call.continueCall(0);
		    			drawable.clearColorFilter();
...
*/
	...
	private void initSpeakerButton() {
        ...
        getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_sound_on).clearColorFilter();
        speakerButton.setBackgroundDrawable(getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_sound_on));
        speakerButton.setOnClickListener(new OnClickListener() {
    		...
    		public void onClick(View arg0) {
    			...
    			Drawable drawable = getResources().getDrawable(foo.bar.otaden.R.drawable.ic_jog_dial_sound_on);
    	    	if (inSpeakerMode) {
...
*/
	...
	private void initSoundPool() {
		...
		soundPool.load(getApplicationContext(), R.raw.boatbell, 1);  // 1, 残り時間が少なくなった時
		soundPool.load(getApplicationContext(), R.raw.ring, 1);      // 2, 着信時
		soundPool.load(getApplicationContext(), R.raw.end, 1);       // 3, 終了時
		return;
	}
    ...
    public void makeCall() {
        ...
        listener = new SipAudioCall.Listener() {
            ...
            public void onCallEstablished(SipAudioCall call) {
            	...
            	if (call.isInCall()) {
                	...
                	// 相手の一言メッセージを取得し、画面に表示する
                	// 通話ログを書き込む
                    if (getMyFavoriteMember() == AppConstants.CATEGORY_MOMO) {
                        // カテゴリーがももクロの時は、もも電互換用のタスクを呼ぶ
                    	new GetProfileMomoTask(callActivity).execute(AppConstants.GET_PROFILE_URL_MOMO, callActivity.call.getPeerProfile().getUserName());
                    	// カテゴリーがももクロの時は、3個目の引数（選択したカテゴリー、passに設定）は9にする
                    	new InsertLogTask().execute(AppConstants.INSERT_LOG_URL, thisDN, 9, getMessage());
                    } else {
                    	new GetProfileTask(callActivity).execute(AppConstants.GET_PROFILE_URL, callActivity.call.getPeerProfile().getUserName());
                    	new InsertLogTask().execute(AppConstants.INSERT_LOG_URL, thisDN, getMyFavoriteMember(), getMessage());
                    }
            ...
            public void onCallBusy(SipAudioCall call) {
                ...
                // カテゴリー、一言メッセージを発信先番号のisUse、commentに書き込む
                // 選択したカテゴリーがももクロの時は、もも電互換用の引数をセットして呼び出す
                if (getMyFavoriteMember() == AppConstants.CATEGORY_MOMO) {
        	        new PutProfileTask().execute(AppConstants.PUT_PROFILE_URL_MOMO, Integer.toString(AppConstants.CALLER_TARGET_DN), 1, getMessage());
                } else {
        	        new PutProfileTask().execute(AppConstants.PUT_PROFILE_URL, getTargetDn(), getMyFavoriteMember(), getMessage());
                }
        ...
        // 音声通話発信
        try {
			call = manager.makeAudioCall(sp.getUriString(), getTargetDn() + "@" + AppConstants.SIPSERVER_IP, listener, 20);
			Log.d(TAG, "target dn = " + getTargetDn());
    ...
    /**
     * 選んだカテゴリーに対応した架電先電話番号を計算する
	 * @return 架電先となる電話番号、5100,5200,,,5800,5900
	 */
	private String getTargetDn() {
		// 選択したカテゴリーがももクロの時は5000番にかける
		if (getMyFavoriteMember() == AppConstants.CATEGORY_MOMO) {
			return Integer.toString(AppConstants.CALLER_TARGET_DN);
		} else {
			int intTargetDn = AppConstants.CALLER_TARGET_DN + getMyFavoriteMember() * 100;
			String strTargetDn = Integer.toString(intTargetDn);
			return strTargetDn;
		}
	}

	/**
     * プリファレンスから保存されているカテゴリーを取得する、未保存の場合は（1:アニメ）にする
	 * @return int 選択したカテゴリーを表す番号、1～3、内容はstring.xmlに記載
	 */
	...
	/**
     * プリファレンスから保存されている一言メッセージを取得する、未保存の場合は空文字列にする
	 * @return 一言メッセージ
	 */
	public String getMessage() {
		SharedPreferences preferences = getSharedPreferences(AppConstants.PREFERENCE_FILE_NAME, MODE_PRIVATE);
	    String message = preferences.getString("COMMENT", "").toString();
	    return message;
	}
...
//					callActivity.updateProgressDialog(i);
	...
	private void releaseDn() {
		...
		if (! thisDN.equals("")) {
            ...
            if (getMyFavoriteMember() == AppConstants.CATEGORY_MOMO) {
    	        new ReleaseDnTask(resultMap).execute(AppConstants.RELEASE_CALLER_DN_URL_MOMO, thisDN);
            } else {
