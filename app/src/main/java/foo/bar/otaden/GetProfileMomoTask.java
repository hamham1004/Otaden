 ...
 */
public class GetProfileMomoTask extends AsyncTask<Object, Void, String> {
    ...
    // コンストラクタ
	public GetProfileMomoTask(CallActivity activity) {
        super();
	...
	protected String doInBackground(Object... params) {
		...
		Log.i(TAG, "url=<" + params[0] + ">, dn=<" + params[1] + ">");
		...
		try {
	        ...
	        if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
	            ...
	            isUse = Integer.parseInt(json.getString("isUse"));
		...
		Log.d(TAG, "isUse=<" + isUse + ">, comment=<" + comment + ">");

		// 相手がもも電から電話している場合は、isUseの値から取得した推しメンをセット
		if (isUse > 0 && isUse < 7 && comment.equals("")) {
			// string.xmlからメンバー名を取得
			String[] memberArray = activity.getResources().getStringArray(R.array.momo_member_name);
			comment = memberArray[isUse - 1];
		}
		
		Log.i(TAG, "doInBackground end, dn=<" + params[1] + ">, result=<" + comment + ">");
		return comment;
	}
